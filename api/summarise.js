// api/summarise.js
import fetch from "node-fetch"; // Vercel serverless uses Node.js
...
export default async function handler(req, res) {
  ...
  if (!text || text.trim().length < 10) {
    return res.status(400).json({ error: "Please provide at least 10 words." });
  }

  const apiKey = process.env.APIKEY; // your Anthropic API key in Vercel env vars
  if (!apiKey) {
    return res.status(500).json({ error: "API key not configured." });
  }

  // Map length/style/focus to prompt text
  const lengthMap = {
  ...
  const styleMap = {
    ...
    simple: "Explain in simple language (ELI5 style).",
  };
